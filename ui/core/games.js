        ...
        return classroomGamesOriginalXhrGET(filename, responsetype, function(response) {
            if (responsetype === 'document' && /toolbox\/esp32\.xml/i.test(filename)) {
                ...
                try {
                    ...
                    var gameBlocks = ['play_invaders_standalone', 'play_snake_standalone', 'play_defender_standalone', 'play_frogger_standalone', 'play_pacman_standalone'];
...
window.addEventListener('load', function () {
    ...
    /* Pacman is standalone: the complete game is emitted into the generated
       Python program, so no pacman.py upload or provisioning change is
       required. */
    var PACMAN_SRC = [
        "def _play_pacman(oled, btn, x0=28, y0=24, led=None):",
        "    # Everything is nested so the generated program gets one global only.",
        "    import time",
        "    import random",
        "    ",
        ...
        "    LIVES = 3",
        "    ",
        "    # overall speed: 1.0 = full, 0.7 = a bit slower",
        "    SPEED = 0.7",
        "    PAC_MS = 55        # ms per pixel",
        "    GHOST_MS = 65",
        "    FRIGHT_GHOST_MS = 110",
        "    FRIGHT_MS = 5000",
        "    GHOST_RESPAWN_MS = 1500",
        "    NGHOST = 2",
        "    ",
        "    # directions and their bit in the open-passage mask",
        "    UP, RIGHT, DOWN, LEFT = 0, 1, 2, 3",
        "    _DX = (0, 1, 0, -1)",
        "    _DY = (-1, 0, 1, 0)",
        "    _BIT = (1, 2, 4, 8)",
        "    ",
        "    # Maze: odd rows/cols are cells ('.' dot, 'o' power pellet, ' ' empty),",
        "    # even positions are wall segments ('#') or passages (' ').",
        "    _MAP = (",
        "        \"###################\",",
        "        \"#  . . . . . . .  #\",",
        "        \"# # ### # # ### # #\",",
        ...
        "            for c in range(COLS):",
        "                m = 0",
        "                if r > 0 and _MAP[2 * r][2 * c + 1] == \" \":",
        "                    m |= 1",
        "                if _MAP[2 * r + 1][2 * c + 2] == \" \":",
        "                    m |= 2",
        "                if r < ROWS - 1 and _MAP[2 * r + 2][2 * c + 1] == \" \":",
        "                    m |= 4",
        "                if _MAP[2 * r + 1][2 * c] == \" \":",
        "                    m |= 8",
        "                orow[c] = m",
        "                ch = line[2 * c + 1]",
        "                drow[c] = 1 if ch == \".\" else 2 if ch == \"o\" else 0",
        "            op.append(orow)",
        "            dots.append(drow)",
        "        return op, dots",
        "    ",
        ...
        "    ",
        "    def _distances():",
        "        \"\"\"All-pairs shortest path (in cells) through the maze, so the ghosts",
        "        chase properly instead of getting stuck behind walls.\"\"\"",
        "        n = ROWS * COLS",
        "        table = []",
        ...
        "    ",
        "    ",
        "    # 6x6 sprites, MSB = leftmost pixel",
        "    _PAC_CLOSED = _fb(b\"\\x78\\xFC\\xFC\\xFC\\xFC\\x78\", 6, 6)",
        "    _PAC_OPEN = (",
        "        _fb(b\"\\x48\\xCC\\xFC\\xFC\\xFC\\x78\", 6, 6),   # up",
        "        _fb(b\"\\x78\\xF8\\xE0\\xE0\\xF8\\x78\", 6, 6),   # right",
        "        _fb(b\"\\x78\\xFC\\xFC\\xFC\\xCC\\x48\", 6, 6),   # down",
        "        _fb(b\"\\x78\\x7C\\x1C\\x1C\\x7C\\x78\", 6, 6),   # left",
        "    )",
        "    _GHOST = _fb(b\"\\x78\\xFC\\xB4\\xFC\\xFC\\xB4\", 6, 6)",
        "    _GHOST_FRIGHT = _fb(b\"\\x78\\x84\\xB4\\x84\\x84\\xA8\", 6, 6)",
        "    ",
        "    # ---------------- shared tuning ----------------",
        "    FRAME_MS = 40          # ~25 fps target; oled.show() is the real throttle",
        "    MAX_STEP_MS = 50       # cap one physics step so a slow frame can't tunnel",
        ...
        "    ",
        "        def _setup(self):",
        "            self.dots = [bytearray(COLS) for _ in range(ROWS)]",
        "            # ghost: [x, y, dir, acc, respawn_at]",
        "            self.ghosts = [[0, 0, LEFT, 0, 0] for _ in range(NGHOST)]",
        "    ",
        "        def reset(self):",
        "            self.score = 0",
        "            self.lives = LIVES",
        ...
        "            self.acc = 0",
        "            self.anim = 0",
        "            self.fright_until = self.now",
        "            self.eaten = 0",
        "            for i in range(NGHOST):",
        "                g = self.ghosts[i]",
        "                g[0] = (3 + 2 * i) * CELL",
        "                g[1] = 2 * CELL",
        "                g[2] = LEFT if i == 0 else RIGHT",
        "                g[3] = 0",
        "                g[4] = time.ticks_add(self.now, 600 * i)",
        "            self.gst = 0",
        "            self.gt0 = self.now",
        "            self.hold_dir = UP",
        "            self.clear_events()",
        ...
        "                f *= 0.9",
        "            return int(base * f / SPEED)",
        "    ",
        "        # ---------------- one-button gesture decoder ----------------",
        "        def _gesture(self):",
        "            \"\"\"Returns a direction or None. States: 0 idle, 1 first press down,",
        "            2 released (waiting for a possible second press), 3 second press",
        "            down, 4 holding.\"\"\"",
        "            now = self.now",
        "            tap = self.take_tap()",
        "            rel = self.take_release()",
        ...
        "    ",
        "        # ---------------- movement ----------------",
        "        def _can_go(self, x, y, d):",
        "            \"\"\"x, y must be cell aligned.\"\"\"",
        "            return _OPEN[y // CELL][x // CELL] & _BIT[d] != 0",
        "    ",
        "        def _step(self, ent, d):",
        "            \"\"\"Move a [x, y, ...] entity one pixel in direction d with tunnel wrap.\"\"\"",
        "            ent[0] += _DX[d]",
        "            ent[1] += _DY[d]",
        "            if ent[0] < 0:",
        "                ent[0] = (COLS - 1) * CELL",
        "            elif ent[0] > (COLS - 1) * CELL:",
        "                ent[0] = 0",
        "    ",
        "        def _move_pac(self, dt_ms):",
        "            self.acc += dt_ms",
        "            step = self._ms(PAC_MS)",
        "            while self.acc >= step:",
        "                self.acc -= step",
        "                aligned = self.px % CELL == 0 and self.py % CELL == 0",
        "                if self.want >= 0:",
        "                    if self.dir >= 0 and self.want == (self.dir + 2) % 4:",
        "                        self.dir = self.want         # reverse any time",
        "                        self.want = -1",
        "                    elif aligned and self._can_go(self.px, self.py, self.want):",
        "                        self.dir = self.want",
        "                        self.want = -1",
        "                if self.dir < 0:",
        "                    return",
        "                if aligned and not self._can_go(self.px, self.py, self.dir):",
        "                    self.dir = -1",
        "                    return",
        "                pac = [self.px, self.py]",
        "                self._step(pac, self.dir)",
        "                self.px, self.py = pac[0], pac[1]",
        "                self.anim += 1",
        ...
        "                        if d == 2:",
        "                            self.score += 50",
        "                            self.fright_until = time.ticks_add(self.now, FRIGHT_MS)",
        "                            self.eaten = 0",
        "                            self.blink(120)",
        "                        else:",
        "                            self.score += 10",
        "                            self.blink(20)",
        "    ",
        "        def _move_ghost(self, g, dt_ms):",
        "            if time.ticks_diff(self.now, g[4]) < 0:",
        "                return",
        "            fright = time.ticks_diff(self.fright_until, self.now) > 0",
        "            g[3] += dt_ms",
        "            step = self._ms(FRIGHT_GHOST_MS if fright else GHOST_MS)",
        "            while g[3] >= step:",
        "                g[3] -= step",
        "                if g[0] % CELL == 0 and g[1] % CELL == 0:",
        "                    opts = []",
        "                    back = (g[2] + 2) % 4",
        "                    for d in range(4):",
        "                        if d != back and self._can_go(g[0], g[1], d):",
        "                            opts.append(d)",
        "                    if not opts:",
        "                        opts.append(back)",
        "                    if fright or random.randint(0, 3) == 0:",
        "                        g[2] = opts[random.randint(0, len(opts) - 1)]",
        "                    else:",
        "                        target = ((self.py + 4) // CELL) * COLS + ((self.px + 4) // CELL) % COLS",
        "                        best = opts[0]",
        "                        bd = 255",
        "                        for d in opts:",
        "                            r = g[1] // CELL + _DY[d]",
        "                            c = (g[0] // CELL + _DX[d]) % COLS",
        "                            dist = _DIST[r * COLS + c][target]",
        "                            if dist < bd:",
        "                                bd = dist",
        "                                best = d",
        "                        g[2] = best",
        ...
        "            if d is not None:",
        "                self.want = d",
        "                if self.dir < 0 and self.px % CELL == 0 and self.py % CELL == 0 \\",
        "                        and self._can_go(self.px, self.py, d):",
        "                    self.dir = d",
        "                    self.want = -1",
        "            self._move_pac(dt_ms)",
        "    ",
        "            if self.ndots == 0:",
        "                self.level += 1",
        "                self.score += 100",
        ...
        "                return 0",
        "    ",
        "            fright = time.ticks_diff(self.fright_until, now) > 0",
        "            for g in self.ghosts:",
        "                self._move_ghost(g, dt_ms)",
        "                if time.ticks_diff(now, g[4]) < 0:",
        "                    continue",
        "                if abs(g[0] - self.px) < 6 and abs(g[1] - self.py) < 6:",
        "                    if fright:",
        "                        self.eaten += 1",
        "                        self.score += 200 * self.eaten",
        "                        self.blink(150)",
        "                        g[0] = 4 * CELL",
        "                        g[1] = 2 * CELL",
        "                        g[4] = time.ticks_add(now, GHOST_RESPAWN_MS)",
        "                    else:",
        "                        return self._die()",
        "            return 0",
        "    ",
        ...
        "                if not (m0 & 8):",
        "                    o.vline(x0, y, CELL, 1)",
        "                if not (_OPEN[r][COLS - 1] & 2):",
        "                    o.vline(x0 + W - 1, y, CELL, 1)",
        "                for c in range(COLS):",
        "                    x = x0 + c * CELL",
        "                    m = _OPEN[r][c]",
        "                    if c < COLS - 1 and not (m & 2):",
        "                        o.vline(x + CELL - 1, y, CELL, 1)",
        "                    if r < ROWS - 1 and not (m & 4):",
        "                        o.hline(x, y + CELL - 1, CELL, 1)",
        "            # dots",
        "            big = (now // 200) & 1",
        "            for r in range(ROWS):",
        "                row = self.dots[r]",
        ...
        "                    d = row[c]",
        "                    if d == 1:",
        "                        o.fill_rect(x0 + c * CELL + 3, y + 3, 2, 2, 1)",
        "                    elif d == 2 and big:",
        "                        o.fill_rect(x0 + c * CELL + 2, y + 2, 4, 4, 1)",
        "            # ghosts",
        "            fright_left = time.ticks_diff(self.fright_until, now)",
        "            for g in self.ghosts:",
        "                if time.ticks_diff(now, g[4]) < 0:",
        "                    continue",
        "                if fright_left > 0 and (fright_left > 1500 or (now // 150) & 1):",
        "                    spr = _GHOST_FRIGHT",
        "                else:",
        "                    spr = _GHOST",
        "                o.blit(spr, x0 + g[0] + 1, y0 + g[1] + 1, 0)",
        "            # pac (flashes while dying)",
        "            if (not self.dying) or (now // 120) & 1:",
        "                if self.dir >= 0 and (self.anim // 3) & 1:",
        "                    spr = _PAC_OPEN[self.dir]",
        "                elif self.dir < 0 and self.want >= 0:",
        "                    spr = _PAC_OPEN[self.want]",
        "                else:",
        "                    spr = _PAC_CLOSED",
        "                o.blit(spr, x0 + self.px + 1, y0 + self.py + 1, 0)",
        "            # score, lives",
        "            _num(o, x0 + 2, y0 + 2, self.score, True)",
        "            for i in range(3 if self.lives > 3 else self.lives):",
        "                o.fill_rect(x0 + W - 4 - i * 4, y0 + 2, 3, 2, 1)",
        "            if self.msg:",
        "                o.fill_rect(x0 + 4, y0 + 15, W - 8, 10, 0)",
        "                o.text(self.msg, x0 + (W - 8 * len(self.msg)) // 2, y0 + 16, 1)",
        "            o.show()",
        ...
        "                last = 0",
        "                scroll = 0",
        "                help_text = \"TAP=L  2TAP=R  HOLD=U  2HOLD=D   \"",
        ...
        "                    if self.take_release():",
        "                        break",
        "                    if time.ticks_diff(self.now, last) >= 80:",
        "                        last = self.now",
        "                        scroll = (scroll + 2) % (W + help_width)",
        "                        blink = (self.now // 400) & 1",
        "                        self.o.fill(0)",
        "                        self.o.text(\"PACMAN\", self.x0 + 12, self.y0, 1)",
        "                        hi_text = \"HI %d\" % self.hi",
        "                        self.o.text(hi_text, self.x0 + (W - 8 * len(hi_text)) // 2, self.y0 + 10, 1)",
        "                        self.o.text(help_text, self.x0 + W - scroll, self.y0 + 20, 1)",
        "                        if blink:",
        "                            self.o.text(\"TAP\", self.x0 + 24, self.y0 + 31, 1)",
        "                        self.o.show()",
        "                    time.sleep_ms(15)",
        "    ",
        "                # play",
    ...
    Blockly.Blocks['play_pacman_standalone'] = {
        init: function() {
            this.setColour(290);
            this.appendDummyInput().appendField('Pacman');
            this.appendDummyInput().appendField(new Blockly.FieldImage("media/pacman.svg", 55, 55, "*"));
            this.appendDummyInput()
                .appendField('SDA').appendField(new Blockly.FieldNumber(5, 0, 48, 1), 'SDA')
                .appendField('SCL').appendField(new Blockly.FieldNumber(6, 0, 48, 1), 'SCL');
            this.setPreviousStatement(true, null);
            // No next connector: the game never returns.
            this.setTooltip('Start Pacman. Single-click BOOT to go left, double-click right, hold to go up, or double-click and hold to go down. This block never finishes - press RST to leave the game.');
        }
    };

    Blockly.Python['play_pacman_standalone'] = function(block) {
        var sda = block.getFieldValue('SDA') || '0';
        var scl = block.getFieldValue('SCL') || '0';

        Blockly.Python.definitions_['import_pacman_sa'] = 'from machine import Pin, I2C\nimport ssd1306';
        Blockly.Python.definitions_['pacman_standalone_src'] = PACMAN_SRC;
        ...
        return  'try:\n' +
                ...
                '    pass\n' +
                'if ' + sda + ' == ' + scl + ':\n' +
                '    raise ValueError("Pacman: SDA and SCL cannot be the same pin")\n' +
                '_pacman_i2c = I2C(0, scl=Pin(' + scl + '), sda=Pin(' + sda + '), freq=400000)\n' +
                '_pacman_oled = ssd1306.SSD1306_I2C(128, 64, _pacman_i2c)\n' +
                '_pacman_led = Pin(8, Pin.OUT)\n' +
                '_pacman_led.value(1)\n' +
                '_play_pacman(_pacman_oled, Pin(9, Pin.IN, Pin.PULL_UP), led=_pacman_led)\n';
    };
});
